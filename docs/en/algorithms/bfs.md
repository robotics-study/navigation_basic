...
**Lemma (queue monotonicity).** Throughout BFS the queue holds depth-$k$ nodes ahead of
depth-$(k{+}1)$ nodes, and the $d$ values dequeued are non-decreasing.

**Theorem.** At the moment $v$ is dequeued, $d[v]=\delta(s,v)$.

*Proof.* Induction on $d$. $d[s]=0=\delta(s,s)$. When some $v$ is dequeued and a neighbor $u$ is
unvisited, set $d[u]:=d[v]+1$ and enqueue it. By FIFO order and the lemma, depth-$k$ nodes are
processed before depth-$(k{+}1)$ nodes, so the **first** time $u$ is reached is along a minimum-hop
path: $d[u]=d[v]+1=\delta(s,v)+1=\delta(s,u)$. Hence the path found when the goal is first reached
is hop-minimal. ∎
...
so BFS gives no world-distance optimality guarantee (its match with Dijkstra in the demo is a
coincidence of this map).

**Complexity.** Each vertex is enqueued/dequeued exactly once ($O(V)$); each adjacency list is
scanned once, so $\sum_v\deg(v)=2E$ — total $O(V+E)$.

## Implementation Notes

- C++: `cpp/src/global_planning/bfs.cpp`, Python: `python/nav_study/global_planning/bfs.py`
- Both implementations depend only on the `DiscreteSpace` capability (`neighbors`, `heuristic`). They know nothing about concrete map types.
- BFS has no tuning parameters (`configs/global_planning/bfs.yaml` is an empty declaration). It is deterministic, so the C++ and Python results match exactly.
