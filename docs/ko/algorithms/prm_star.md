...
- 로드맵·연결·Dijkstra 질의는 [PRM](prm.md) 와 공유하는 공통 유틸(`roadmap_common` / `_roadmap`)에
  있다. PRM\* 은 반경 정책만 다르다.
- 줄어드는 반경 $r_n=\gamma\sqrt{\log n/n}$ 은 [FMT\*](fmt_star.md)·[BIT\*](bit_star.md) 와 공유하는
  `sampling_common` / `_sampling` 의 `rgg_radius` 로 계산한다.
