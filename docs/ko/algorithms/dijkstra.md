...
DIJKSTRA(start, goal):
    ...
    while open is not empty:
        ...
        if v already settled: continue        # lazy deletion (아래 증명의 따름정리)
...
**보조정리 0 (상계 불변식).** 실행 전 과정에서 임의 노드 $v$ 에 대해 $g[v]\ge\delta(s,v)$ 이고,
$g[v]<\infty$ 이면 그 값은 **실재하는** $s\to v$ 경로 하나의 비용이다.

*증명.* relaxation 은 $g[u]\leftarrow g[v]+c(v,u)$ 로만 값을 낮추는데, 이는 "$s\to v$ 실재 경로 +
간선 $(v,u)$" 라는 실재 경로의 비용이므로 항상 어떤 실재 경로 비용 $\ge\delta(s,u)$ 다. 초기값
$g[s]=0=\delta(s,s)$, 나머지 $\infty$ 도 조건을 만족. ∎

**정리 (확정 시 정확).** 우선순위 큐가 $u$ 를 (최소 $g$ 로) 꺼내 확정하는 시점에
$g[u]=\delta(s,u)$ 이다.
...
(첫 부등식에는 보조정리 0 의 $g[y]\ge\delta(s,y)$ 가 함께 쓰여 $g[y]=\delta(s,y)$ 로 조인다).
비음수 가정은 $\delta(s,y)\le\delta(s,u)$ 단계에서 필수적이다 (음수 간선이면 성립하지 않음 →
Bellman–Ford 영역). ∎

**따름정리 (확정 후 불변 → lazy deletion 정당성).** 노드가 확정되면 그 $g$ 는 이미
$\delta(s,\cdot)$ 이고 보조정리 0 으로 더 낮아질 수 없다. 따라서 큐에 남은 그 노드의 **오래된
중복 항목**을 pop 할 때 건너뛰어도(= lazy deletion) 최적성이 깨지지 않는다 — decrease-key 자료구조
없이 중복 삽입만으로 정확성이 보장된다.

**복잡도.** binary heap 기준 extract-min $V$ 회 $O(V\log V)$, 완화마다 push $E$ 회
$O(E\log V)$ → $O((V+E)\log V)$. lazy 중복 삽입으로 힙 크기가 $O(E)$ 까지 커질 수 있으나 각 항목이
$O(\log E)=O(\log V)$ 라 점근 차수는 그대로다.
